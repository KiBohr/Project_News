...
async function fetchNewsData(
  ...
  try {
    const newsData = await ky
      .get<newsData>(
        `https://newsapi.org/v2/everything?q=${searchTerm}&sorttBy=${sortBy}&${language}=en&apiKey=${apiKey}`
      )
      .json();
